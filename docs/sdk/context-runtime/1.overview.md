...
description: Architecture overview of the CLIO Runtime for developers.
---

# CLIO Runtime Overview

This section is aimed at developers building modules (ChiMods) or integrating with the CLIO Runtime programmatically.

## What is the CLIO Runtime?

The CLIO Runtime is the task-execution engine at the core of IOWarp. It provides:
...
Applications never call Module code directly. Instead they submit **tasks** via a client library; the runtime schedules and executes them.

## Core Abstractions

### Pools and Containers

A **pool** is a named, addressable group of **containers** spread across one or more nodes. Every Module instance is backed by a pool. Pools are identified by a `PoolId` (e.g., `"512.0"`) and a human-readable `pool_name`.

Each pool is made up of one or more **containers**, numbered sequentially from `0` to `N-1`. A container is a single instance of a Module on a specific node. Each container maps to exactly one node, but a single node may host multiple containers from the same pool.
...
When a task's `PoolQuery` resolves to a remote node, the runtime serializes the task and sends it over ZeroMQ (or Unix domain socket, depending on `CLIO_IPC_MODE`). The remote node executes the task and returns the result. This is transparent to the client.
...
- [Module Development Guide](./2.module_dev_guide.md) — Build your own Module.
