...
version: 1.0.0
description: "Chimaera Runtime Module Repository"
...
Chimaera uses C++20 coroutines to enable cooperative task execution within runtime methods. This section covers the coroutine primitives and patterns used in Module development.
...
- Automatically links core Chimaera library (`chimaera::cxx` or `ctp::cxx`)
...
External applications linking against Module libraries receive all necessary dependencies automatically. The Module client libraries include the core Chimaera library as a transitive dependency.
...
Each Module's `CMakeLists.txt` uses the standard Chimaera build utilities:
...
# Core Chimaera library is automatically included by Module dependencies
```
