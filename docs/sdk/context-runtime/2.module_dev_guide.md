...
**CRITICAL**: All Module clients implementing Create functions MUST use the explicit `chi::kAdminPoolId` variable when constructing CreateTask operations. You CANNOT use `pool_id_` for CreateTask operations.
...
// Define Module entry points using CHI_TASK_CC macro
...
The CLIO Runtime supports two execution modes through the `ExecMode` enum in `RunContext`, enabling sophisticated task routing patterns:
...
The admin Module's `GetOrCreatePool` method demonstrates the canonical dynamic scheduling pattern:
...
void Runtime::GetOrCreatePool(
  ...
  HLOG(kDebug, "Admin: Executing GetOrCreatePool task - Module: {}, Pool: {}",
...
struct BaseCreateTask : public chi::Task {
  ...
  INOUT chi::string chimod_name_;     // Module name for loading
...
CLIO Runtime uses an **async-only client API pattern**. All client operations are asynchronous, returning `chi::Future<TaskType>` objects. This design:
...
```cpp
/**
 * Auto-generated execution implementation for MOD_NAME Module
...
CLIO Runtime provides a unified initialization function `CHIMAERA_INIT()` that supports different operational modes:
...
Module entry points are now hidden behind the `CHI_TASK_CC` macro:
...
```yaml
compose:
- mod_name: chimaera_bdev        # Module library name
