...
description: How CUDA/ROCm kernels submit tasks to the Clio runtime and receive results.
---

# GPU Client Kernels

This guide explains how CUDA or ROCm kernels can submit tasks to the Clio runtime and receive results, using the same client API as host code. It also covers **cross-device routing**: CPU code sending tasks to run on the GPU, and GPU kernels sending tasks to run on the CPU.

## Overview

The Clio runtime supports four GPU task routing modes:

| Routing Mode | Direction | Description |
|---|---|---|
| `PoolQuery::Local()` | GPU &rarr; GPU | GPU kernel submits a task processed by the GPU work orchestrator |
| `PoolQuery::LocalGpuBcast()` | CPU &rarr; GPU | CPU submits a task processed by the GPU work orchestrator |
| `PoolQuery::ToLocalGpu(gpu_id)` | CPU &rarr; GPU | CPU submits a task to a specific GPU device |
| `PoolQuery::ToLocalCpu()` | GPU &rarr; CPU | GPU kernel submits a task processed by a CPU worker |
...
The Clio runtime supports two process configurations for GPU task submission:
...
- **TCP/IPC mode**: `FutureShm` allocated via `CTP_MALLOC` (process-local heap). Task data serialized and sent through ZMQ DEALER socket.
...
**Primary backend** (`backend`): `GpuShmMmap` pinned host memory (or `GpuMalloc` device memory for GPU→GPU). Provides per-thread `ArenaAllocator` (`CTP_DEFAULT_ALLOC_GPU_T`) for fast bump-pointer allocation of `FutureShm` and task objects.
...
```
 backend (ArenaAllocator, CTP_DEFAULT_ALLOC_GPU_T):
 +------------------------------------------------------+
...
1. Query `ctp::GpuApi::GetDeviceCount()` for number of GPU devices
...
The pure client's transport mode is determined by the `CLIO_IPC_MODE` environment variable or `chimaera.yaml` configuration:
...
| `TCP` | ZMQ DEALER/ROUTER | Process-local heap (`CTP_MALLOC`) | Serialized through ZMQ TCP socket |
| `IPC` | ZMQ DEALER/ROUTER (Unix socket) | Process-local heap (`CTP_MALLOC`) | Serialized through ZMQ IPC socket |
...
These set the `CTP_ENABLE_CUDA` or `CTP_ENABLE_ROCM` preprocessor macros, which gate all GPU code paths.
...
1. Mark constructors and serialization methods with `CTP_CROSS_FUN`
...
struct MyGpuTask : public chi::Task {
  ...
  CTP_CROSS_FUN MyGpuTask() : chi::Task(), input_value_(0), result_(0) {}

  CTP_CROSS_FUN explicit MyGpuTask(
...
**Important**: Every function called during serialization must be `CTP_CROSS_FUN` (i.e., `__host__ __device__`). Without this annotation, NVCC's SFINAE-based dispatch silently skips serialization on the GPU, producing corrupted data with no compiler error.

## Writing a GPU-Compatible Client

The client class needs `CTP_CROSS_FUN` on the constructor and any methods called from GPU code:
...
- **GPU device code** (`CTP_IS_GPU`): routes via `SendGpu` or `SendGpuLocal`
- **CPU host code** with `CTP_ENABLE_CUDA=1`: detects `LocalGpuBcast`/`ToLocalGpu` and routes via `SendToGpu`
...
```cpp
// Host code -- must be compiled with CTP_ENABLE_CUDA=1
chimaera::my_module::Client client(pool_id);
...
**Important -- CTP_ENABLE_CUDA must be 1**: The `Send()` GPU routing interceptor is inside `#if CTP_ENABLE_CUDA || CTP_ENABLE_ROCM`. If your source file is compiled with `CTP_ENABLE_CUDA=0` (e.g., a plain C++ file in a mixed CUDA/C++ target), the interceptor is compiled out and the task will go through normal CPU routing, which does not complete the future correctly for GPU tasks.
...
class GpuRuntime : public chi::gpu::Container {
 public:
  CTP_GPU_FUN GpuRuntime() = default;
  CTP_GPU_FUN ~GpuRuntime() override = default;

  /** GPU handler for MyMethod */
  CTP_GPU_FUN void MyMethod(hipc::FullPtr<MyGpuTask> task,
...
Enable GPU support in your module's `clio_mod.yaml`:
...
Run `clio_run repo refresh .` to regenerate the autogen dispatch code:
...
**Important**: Use stream-based synchronization (`ctp::GpuApi::Synchronize(stream)`) for your own kernels, not `cudaDeviceSynchronize()`. The latter would block on the orchestrator's stream.
...
```cpp
// Compile this in a CUDA source file (CTP_ENABLE_CUDA=1)
chi::CHIMAERA_INIT(chi::ChimaeraMode::kClient, true);
...
CTP_CROSS_FUN size_t GetCopySpaceSize() const { return 8192; }
```
...
The Clio runtime automatically detects GPU devices and creates GPU queues during `ServerInit`. The default scheduler (`DefaultScheduler`) partitions workers as follows:
...
target_link_libraries(my_gpu_kernels PRIVATE ctp::cuda_cxx)

# Main executable with CPU sources
add_executable(my_app my_main.cc $<TARGET_OBJECTS:my_gpu_kernels>)

# CRITICAL: CPU sources are compiled with CTP_ENABLE_CUDA=0 to avoid
...
```cpp
// my_gpu_kernels.cc (compiled as CUDA, CTP_ENABLE_CUDA=1)
...
The test file `context-runtime/modules/MOD_NAME/test/test_gpu_client_process.cc` verifies cross-process GPU task submission using the fork-based test pattern. A child process runs as a standalone Clio server (with `--server-mode`), while the parent connects as a pure client.

### Test Infrastructure

All tests share the same server/client lifecycle managed by `EnsureInitialized()`:

1. **`StartServerProcess()`**: Forks a child, sets `CLIO_WITH_RUNTIME=1`, and `execl`s back into the same binary with `--server-mode`. The child calls `CHIMAERA_INIT(kServer, true)` and sleeps until killed.
...
3. **Parent client init**: Sets `CLIO_WITH_RUNTIME=0` and calls `CHIMAERA_INIT(kClient, false)`. During `ClientConnect`, the client receives GPU queue metadata from the server.
...
Requires `CTP_ENABLE_CUDA` or `CTP_ENABLE_ROCM` at compile time. Without GPU support, prints "GPU support not compiled, skipping" and passes.
...
The default `GpuShmMmap` orchestrator scratch (`CTP_DEFAULT_ALLOC_GPU_T = ArenaAllocator`) is reset after each completed task (`alloc->Reset()`), so it never exhausts regardless of task count. The `BuddyAllocator` heap (`CHI_GPU_HEAP_T`) reclaims memory individually as serialization vectors go out of scope.
...
Check that **all** serialization functions are marked `CTP_CROSS_FUN`. Without `__device__` annotation, NVCC's SFINAE dispatch silently skips serialization, consuming 0 bytes and misaligning the stream. The compiler produces no error.
...
- Verify the call site is compiled with `CTP_ENABLE_CUDA=1` -- if compiled with `CTP_ENABLE_CUDA=0`, the `Send()` GPU interceptor is absent and the task goes through normal CPU routing, which cannot complete a GPU-bound future
