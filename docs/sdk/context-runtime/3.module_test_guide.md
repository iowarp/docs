...
export CLIO_SERVER_CONF="/path/to/chimaera_default.yaml"
```

**Module Discovery**: The runtime scans both `CLIO_REPO_PATH` and `LD_LIBRARY_PATH` for Module shared libraries (e.g., `libchimaera_bdev.so`). Point these at your `build/bin` directory.
