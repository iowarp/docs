...
- **Dashboard page**: every bdev pool has a page at `/viz/clio_bdev/` on the runtime's [web dashboard](../../../deployment/monitoring#runtime-dashboard) (capacity meter plus the full stats table), and new bdev pools can be created from the dashboard's **Add Pool** form
