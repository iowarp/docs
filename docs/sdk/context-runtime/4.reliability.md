...
This document describes the algorithms and mechanisms that keep a CLIO Runtime
...
// Override Container::Recover to rebuild state after a node failure. The
// container is recreated on a DIFFERENT node during RecoverContainers.
// The base default just calls Init (clean slate).
struct RecoverExample : public Container {
  void Recover(const PoolId& pool_id, const std::string& pool_name,
               u32 container_id = 0) override {
    Init(pool_id, pool_name, container_id);
  }
  // Other pure-virtual Container methods (Run, SaveTask, ...) omitted.
};
```
...
// Override Container::Restart for a warm start on the SAME node. Rebuilds
// metadata only (data assumed intact on local storage). Default calls Init.
struct RestartExample : public Container {
  void Restart(const PoolId& pool_id, const std::string& pool_name,
               u32 container_id = 0) override {
    Init(pool_id, pool_name, container_id);
  }
  // Other pure-virtual Container methods omitted.
};
```
...
// Override Container::ScheduleRecover to steer recovery placement. Called on
// the leader's local_container_ during ComputeRecoveryPlan. Return a specific
// node id, or static_cast<u32>(-1) to fall back to round-robin.
struct ScheduleRecoverExample : public Container {
  u32 ScheduleRecover() override {
...
// Override Container::Migrate to serialize and transfer container state to the
// destination node. Called on the SOURCE node after plug and drain. The base
// default is a no-op.
struct MigrateExample : public Container {
  void Migrate(u32 dest_node_id) override {
...
// Defined in ipc_run2run.h. The queued task is held by an RAII
// clio::run::shared_ptr (not a raw FullPtr), so it is freed automatically once
// the retry succeeds, permanently fails, or is re-routed.
struct RetryEntry {
...
std::deque<RetryEntry> send_in_retry_;    // failed input sends
std::deque<RetryEntry> send_out_retry_;   // failed output sends
```
...
|---------------------|---------|-------------|
| `CLIO_CLIENT_RETRY_TIMEOUT` | 60.0 | Max seconds to wait for server restart before giving up |
| `CLIO_WAIT_SERVER` | 30 | Initial connection timeout (also used during reconnect) |
